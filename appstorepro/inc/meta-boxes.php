...
function appstorepro_app_details_callback( $post ) {
	...
	echo '<tr>';
	echo '<th><label for="app_screenshots">' . esc_html__( 'Screenshot URLs (one per line)', 'appstorepro' ) . '</label></th>';
	...
	echo '<tr>';
	echo '<th><label for="app_category_icon">' . esc_html__( 'Category Icon', 'appstorepro' ) . '</label></th>';
	...
	echo '<tr>';
	echo '<th><label for="app_is_mod">' . esc_html__( 'Is MOD APK?', 'appstorepro' ) . '</label></th>';
...
function appstorepro_save_app_meta( $post_id ) {
	...
	if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['appstorepro_app_details_nonce'] ) ), 'appstorepro_app_details_save' ) ) {
		return;
	}
