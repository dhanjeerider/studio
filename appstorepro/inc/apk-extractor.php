...
function appstorepro_ajax_scrape_playstore() {
    ...
    if ( ! $url ) {
        wp_send_json_error( [ 'message' => __( 'Missing or invalid URL.', 'appstorepro' ) ] );
    }
    ...
    // by appstorepro_fetch_liteapks_data(). Feel free to add additional
    // scrapers here following the same pattern. If none match, we return
    // an unsupported URL error.
    if ( preg_match( '#play\.google\.com/store/apps/details#', $url ) ) {
        ...
        wp_send_json_error( [ 'message' => __( 'Unsupported URL. Currently we can only scrape Google Play and LiteAPKs links.', 'appstorepro' ) ] );
    }
...
function appstorepro_fetch_liteapks_data( $url ) {
    ...
    if ( is_wp_error( $response ) ) {
        return new WP_Error( 'fetch_failed', sprintf( __( 'Failed to fetch LiteAPKs page: %s', 'appstorepro' ), $response->get_error_message() ) );
    }
    $code = wp_remote_retrieve_response_code( $response );
    if ( 200 !== (int) $code ) {
        return new WP_Error( 'bad_response', sprintf( __( 'LiteAPKs returned HTTP %s. The URL may be invalid or blocked.', 'appstorepro' ), (int) $code ) );
    }
    $html = wp_remote_retrieve_body( $response );
    if ( ! $html ) {
        return new WP_Error( 'empty_body', __( 'Empty response from LiteAPKs.', 'appstorepro' ) );
    }
