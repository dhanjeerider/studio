...
(function ($) {
  ...
  function isValidPlayStoreUrl(url) {
    ...
    // Allow Play Store URLs along with selected third‑party mirrors. The Play Store
    // scraper handles google.com links, while the server‑side logic can also
    // scrape pages from supported sites such as liteapks.com and apkvisit.com.
    // Additional domains can be added to this regex as needed.
    return /play\.google\.com\/store\/apps\/details/.test(url)
        || /liteapks\.com\//.test(url)
        || /apkvisit\.com\//.test(url);
  }
