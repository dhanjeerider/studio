...
$header_main_menu_location   = has_nav_menu( 'header-main-menu' ) ? 'header-main-menu' : '';
$header_custom_menu_location = has_nav_menu( 'header-custom-menu' ) ? 'header-custom-menu' : '';
$header_ad_code = get_theme_mod( 'aspv5_ad_header', '' );
?>
<header class="site-header sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 transition-shadow duration-300" id="aspv5-header">
	<div class="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
		...
		<!-- Desktop Nav -->
		<div class="hidden lg:flex items-center gap-3 min-w-0 flex-1 justify-center">
			<nav class="flex items-center gap-1" aria-label="<?php esc_attr_e( 'Primary navigation', 'aspv5' ); ?>">
				<?php if ( $header_main_menu_location ) : ?>
					<?php
					wp_nav_menu( [
						'theme_location' => $header_main_menu_location,
			...
			<?php if ( $header_custom_menu_location ) : ?>
				<nav class="hidden xl:flex items-center gap-1 aspv5-header-custom-nav" aria-label="<?php esc_attr_e( 'Header custom links', 'aspv5' ); ?>">
					<?php
					wp_nav_menu( [
						'theme_location' => $header_custom_menu_location,
						'menu_class'     => 'flex items-center gap-1',
						'fallback_cb'    => false,
						'depth'          => 1,
						'walker'         => false,
	...
	<?php if ( $header_ad_code ) : ?>
		<div class="max-w-6xl mx-auto px-4 pb-3 aspv5-ad-slot aspv5-ad-header">
			<?php echo do_shortcode( wp_kses_post( $header_ad_code ) ); ?>
		</div>
	<?php endif; ?>
</header>
