...
$mobile_main_menu_location   = has_nav_menu( 'header-main-menu' ) ? 'header-main-menu' : '';
$mobile_custom_menu_location = has_nav_menu( 'header-custom-menu' ) ? 'header-custom-menu' : '';
?>
<!-- Slide Menu Overlay -->
<div class="fixed inset-0 bg-black/50 z-[60] hidden" id="aspv5-menu-overlay" aria-hidden="true"></div>

<!-- Slide Menu Panel -->
<div class="fixed top-0 right-0 bottom-0 w-80 max-w-full bg-white dark:bg-gray-900 z-[70] flex flex-col shadow-2xl"
     id="aspv5-slide-menu"
     role="dialog"
     aria-modal="true"
	...
	<div class="flex-1 overflow-y-auto px-4 py-4">
		...
		<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="mb-4">
			<div class="relative">
				<input type="search" name="s" value="<?php echo esc_attr( get_search_query() ); ?>"
				       placeholder="<?php esc_attr_e( 'Search apps or games...', 'aspv5' ); ?>"
				       class="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 pl-10 pr-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary/40">
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2">
					<circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
				</svg>
			</div>
		</form>

		<p class="px-1 mb-2 text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider"><?php esc_html_e( 'Main Menu', 'aspv5' ); ?></p>
		<div class="space-y-1">
			<?php if ( $mobile_main_menu_location ) : ?>
				<?php
				wp_nav_menu( [
					'theme_location' => $mobile_main_menu_location,
		...
		<?php if ( $mobile_custom_menu_location ) : ?>
			<div class="border-t border-gray-100 dark:border-gray-800 my-3"></div>
			<p class="px-1 mb-2 text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider"><?php esc_html_e( 'Custom Links', 'aspv5' ); ?></p>
			<?php
			wp_nav_menu( [
				'theme_location' => $mobile_custom_menu_location,
				'menu_class'     => 'space-y-0.5',
				'fallback_cb'    => false,
				'depth'          => 1,
				'container'      => false,
