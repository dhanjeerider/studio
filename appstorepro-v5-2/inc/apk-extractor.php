...
function appstorepro_ajax_create_app_post() {
    ...
    $meta_map = [
		...
		'_app_download_url'   => $app['download_url'] ?? '',
...
function appstorepro_parse_playstore_html( $html, $original_url ) {
    $data = [
		...
		'hero'            => '',
        'trailer'         => '',
			'download_url'    => '', // Added download_url field
			'screenshots'     => [],
		'category'        => '',
		'size'            => '',
		'downloads'       => '',
...
function appstorepro_fetch_liteapks_data( $url ) {
	...
	// Download button from LiteAPKs CTA.
	if ( preg_match( '#<a[^>]+class="[^"]*btn-install[^"]*"[^>]+href="([^"]+)"[^>]*>\s*Download APK\s*</a>#si', $html, $m ) ) {
		$data['download_url'] = html_entity_decode( trim( $m[1] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
	} elseif ( preg_match( '#<a[^>]+href="([^"]+)"[^>]*>\s*(?:<svg[^>]*>.*?</svg>\s*)?\s*Download APK\s*</a>#si', $html, $m ) ) {
		$data['download_url'] = html_entity_decode( trim( $m[1] ), ENT_QUOTES | ENT_HTML5, 'UTF-8' );
	}
